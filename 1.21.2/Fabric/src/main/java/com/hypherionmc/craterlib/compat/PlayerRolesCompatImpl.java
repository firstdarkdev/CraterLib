...
public class PlayerRolesCompatImpl implements PlayerRolesCompat {
    ...
    public List<String> getRoles(BridgedGameProfile profile) {
        ...
        var roles = manager.peekRoles(CommonPlatform.INSTANCE.getMCServer().toMojang(), profile.getId()).stream().toList();
    ...
    private void updateRoles(BiPredicate<PlayerRoleSet, SimpleRole> apply, BridgedGameProfile profile, String roleName) {
        ...
        if (role == null) return;
        manager.updateRoles(CommonPlatform.INSTANCE.getMCServer().toMojang(), profile.getId(), roles -> apply.test(roles, role));
    }
