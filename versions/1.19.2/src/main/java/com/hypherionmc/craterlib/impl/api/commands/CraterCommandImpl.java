...
public class CraterCommandImpl implements CraterCommand {
    ...
    public CraterCommand withGameProfilesArgument(String key, CommandExecutorWithArgs<List<? extends CraterGameProfile>> executor) {
        this.mojangCommand.then(Commands.argument(key, GameProfileArgument.gameProfile())
                .executes(context -> executor.run(
                        BridgedPlayer.wrap(context.getSource().getPlayerOrException()),
                        GameProfileArgument.getGameProfiles(context, key).stream().map(BridgedGameProfile::wrap).toList(),
    ...
    private boolean checkPermission(CommandSourceStack stack) {
        try {
            if (!CraterLoader.isModLoaded("luckperms") || CraterLoader.getLoaderType() == LoaderType.PAPER || !(stack.getEntity() instanceof ServerPlayer) || luckPermNode.isEmpty())
                return stack.hasPermission(this.permLevel);

            return LuckPermsCompatImpl.INSTANCE.hasPermission((ServerPlayer) stack.getEntity(), this.luckPermNode) || stack.hasPermission(this.permLevel);
        } catch (Exception e) {
            return stack.hasPermission(permLevel);
        }
    }
