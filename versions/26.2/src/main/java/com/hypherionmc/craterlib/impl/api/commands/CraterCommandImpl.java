...
public class CraterCommandImpl implements CraterCommand {
    ...
    private boolean checkPermission(CommandSourceStack stack) {
        try {
            ...
            return LuckPermsCompatImpl.INSTANCE.hasPermission(stack.getPlayer(), this.luckPermNode) || stack.permissions().hasPermission(new Permission.HasCommandLevel(PermissionLevel.byId(this.permLevel)));
        } catch (Exception e) {
            return stack.permissions().hasPermission(new Permission.HasCommandLevel(PermissionLevel.byId(this.permLevel)));
        }
    }
